...
@keyframes animate {
  ...
  <body class="bg-gray-700 animate-gradient min-w-screen min-h-screen flex items-center justify-center">
...
writeHttpResponse() {
  ...
  if directive_test=$(head -1 "pages/${ROUTE_SCRIPT}"); then
    if [[ "$directive_test" == "# sse" ]]; then
      ...
      CUSTOM_HEADERS=1
    elif [[ "$directive_test" == "# plain" ]]; then
      CONTENT_TYPE="text/plain; charset=utf-8"
    elif [[ "$directive_test" == "# png" ]]; then
  ...
  [[ -z $CUSTOM_HEADERS ]] && header Content-Type "$CONTENT_TYPE" && end_headers
  printf "%s" "$result"
}
